...
### Promotion PR from staging to main

Treat promotion PR creation and merge as separate permissions. When the user
explicitly requests a promotion PR, create only a PR whose head is `staging`
and base is `main`.

Before creating it:

- confirm that the remote `staging` and `main` branches exist and that the
  inspected `main...staging` diff is not empty;
- check for an existing open `staging` to `main` PR and report it instead of
...
1. purpose;
2. changed contents derived from the inspected `main...staging` diff at creation
